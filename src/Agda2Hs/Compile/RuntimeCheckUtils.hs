...
checkRtc' idcs tel = do
  ...
  (belowIdcs,)
    <$> if all isJust belowChks && all isJust ourChks
      then -- all checkable or none erased
...
checkRtc'' (goIdx, argIdx) (d, tUpTo) = do
  ...
  case chk of
    ...
    Checkable' {..} -> do
      ...
      -- e.g. `up x a2`, continuing the example above `NameIndices`
      let lhs = hsPat up : theirLhs
      -- e.g. `up x (go0 a2)`
      let rhs = hsVar up `eApp` theirRhs
      -- Create run time check function which we will only use if we cannot inline
      let maybeRtc = createRtc (hsName go) lhs theirChks rhs $ binds theirDecls
          (ourGoIdx, ourRhs, ourRtc) =
            if null theirChks
              ...
              -- e.g. `\ a1 -> a0 (go1 a1)`
                ...
                (succ theirGoIdx, hsVar go `eApp` [hsVar arg], [maybeRtc])
